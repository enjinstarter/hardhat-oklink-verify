...
describe("Plugin integration tests", function () {
  describe("Using a normal Hardhat project", function () {
    ...
    before(function () {
      if (process.env.RUN_OKLINK_TESTS !== "yes") {
        this.skip();
      } else {
        if (
    ...
    it("Should verify deployed inner contract on OKLink", async function () {
      ...
      return this.env.run("oklink-verify", {
        address: deployedAddress,
      });
    });

    it("Should verify deployed contract with import on OKLink", async function () {
      const deployedAddress = await deployContract(
        "TestReentrancyGuardImported",
        [],
        this.env
      );
    ...
    it("Should verify deployed contract with name clash on OKLink", async function () {
      const deployedAddress = await deployContract(
    ...
    it("Should verify deployed library on OKLink", async function () {
    ...
    it("Should verify deployed contract with a complex parameter list on OKLink", async function () {
    ...
    describe("without a libraries parameter", function () {
      ...
      it("should verify deployed contract with constructor parameter list when library parameter was not provided and libraries are automatically detectable", async function () {
        const deployedLib = await deployContract("NormalLib", [], this.env, 2);

        const aNumber = 50;
        const deployedAddress = await deployContract(
          "OnlyNormalLibWithConstructor",
          [aNumber],
          this.env,
          undefined,
          { libraries: { NormalLib: deployedLib } }
        ...
        return this.env.run("oklink-verify:verify", {
          address: deployedAddress,
    ...
    describe("with a libraries parameter", function () {
      it("should verify deployed contract with constructor parameter list when normal library address is provided and libraries are automatically detectable", async function () {
        const deployedLib = await deployContract("NormalLib", [], this.env, 2);
      ...
      // This test can only fail with the response from OKLink
        ...
        return this.env
          ...
          .catch((reason) => {
            expect(reason).to.be.an.instanceOf(
              ...
              "The OKLink backend should fail this verification"
            );
  ...
  describe("Using a Hardhat project that mocks the rinkeby network", function () {
    ...
    beforeEach(async function () {
      ...
      task(TASK_VERIFY_GET_OKLINK_ENDPOINT).setAction(async () => {
